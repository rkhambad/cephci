...
def image_metadata_feature_disable(rbd, pool_type, **kw):
    ...
    feature_name = kw["config"]["image_feature_name"]
    ...
    try:
        ...
        rbd.image_meta(action="set", image_spec=image_spec, key="last_update", value=value)

        # verify meta value is set on an image
        if value != rbd.image_meta(action="get", image_spec=image_spec, key="last_update")[:-1]:
            log.error(f"Meta value did not set on {image_spec}")
            return 1

        with parallel() as p:
            p.spawn(run_fio, image_name=image, pool_name=pool, client_node=client)
        ...
        if feature_name in image_load.get("features", {}):
            ...
            return 1
        log.info(f"{feature_name} on {image} is disabled successfully")
        return 0
...
def run(**kw):
    ...
    """
    log.info("CEPH-9864 - Running test to verify disable image feature with meta set on it")
